...
func insertDocument(db string, table string, obj map[string]interface{}) {
	...
	idStr, _ := id.(string)
	// 正常情况
	res, err := esClient.Index().Index(indexName).Type(typeName).Id(idStr).BodyJson(obj).Do(context.Background())
	if err != nil {
		// handle error
		return
	}
...
func deleteDocument(
	...
	idStr, _ := id.(string)
	res, err := esClient.Delete().Index(indexName).Type(typeName).Id(idStr).Do(context.Background())
	if err != nil {
		// handle error
		return
	}
